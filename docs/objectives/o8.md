...
Updating the previous command to `grep 18.222.86.32 weberror.log | grep -v ' 404 -'` will filter out all 404 Not Found entries and allow us to locate the first successful request on or after 16:47:45, which appears to be `/proc`.
