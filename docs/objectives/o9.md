...
**Direct link**: [minedoor website](https://hhc22-novel.kringlecon.com?&challenge=minedoor&id=e4ea87e3-7969-4a49-b110-8882b7e1ff2c")
...
We could leverage the previous `canvas` solution by first making lock 5 render the image we need for lock 6 (see below), copying the image link, and submitting `<img src="images/replace_with_proper_filename.png">` on the final lock.

??? example "JavaScript - Draw lock 6 image"
    ```javascript linenums="1"
    <script>
    canvas = document.createElement('canvas');
    canvas.width = 200;
