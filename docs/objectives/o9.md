...
By submitting `<script>document.body.style.backgroundColor="blue";</script>` we can change the `body` element background to blue. This will cause the server to render a blue image that covers the whole lock, creating a path between input and output.
...
We could reuse the previous `canvas` solution by first making lock 1 render the image we need for lock 6, copying the rendered image file name, and submitting `<img src="images/rendered_image_file.png">` on the final lock. Instead, let's opt for a technique we haven't used before and which can be applied directly on lock 6 without the need to leverage any additional locks.

Images can be embedded in an HTML page by putting their [Base64-encoded image data](https://www.w3docs.com/snippets/html/how-to-display-base64-images-in-html.html) in the `src` attribute of an `img` element. Create a PNG image in your preferred image editor, use the [`base64`](https://linuxhint.com/bash_base64_encode_decode/) command or [CyberChef](https://gchq.github.io/CyberChef/) to generate the Base64-encoded image data, and submit it on the lock using `<img src="data:image/png;base64,iVBORw0KGgoAAAANSU...>`. Expand the section below for an example, based on [this PNG image](../artifacts/objectives/o9/red-blue-green.png), which can be copy/pasted directly onto the final lock.
