...
By submitting `<script>document.body.style.backgroundColor="blue";</script>` we can change the html `body` background to blue. This will cause the server to render a blue square that covers the whole lock, creating a path between input and output.
